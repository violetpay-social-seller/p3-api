...
import io.point3.p3api.inquiry.application.submit.OrderFormSubmissionCreateUseCase;
...
public class OrderFormDraftConsumeService implements OrderFormDraftConsumeUseCase{
    ...
    private final OrderFormDraftQueryUseCase orderFormDraftQueryUseCase;
    ...
    public OrderFormDraftConsumeResult consume(ConsumeOrderFormDraftCommand command) {
        ...
        OrderFormDraftData draft = orderFormDraftQueryUseCase.get(command.draftKey());

        // 사전 제출 데이터의 스토어ID와 요청 커맨드의 구매자ID를 통해 채팅방 개설
        Inquiry inquiry = openInquiryUseCase.open(OpenInquiryCommand.of(draft.storeId(), command.buyerUserId()));

        // 사전 제출 데이터를 주문서제출 커맨드와 매핑하여 주문서 생성
        OrderFormSubmission submission = orderFormSubmissionCreateUseCase.create(toCreateSubmissionCommand(draft, command.buyerUserId(), inquiry.getId()));
